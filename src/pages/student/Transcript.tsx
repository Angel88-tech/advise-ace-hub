...
export default function Transcript() {
  ...
  const getTranscriptUrl = (path: string) => {
    return supabase.storage.from(TRANSCRIPTS_BUCKET).getPublicUrl(path).data.publicUrl
  }
  ...
  const loadAcademicProfile = async () => {
    ...
    if (data) {
      ...
      setSavedFilePath(data.transcript_file_path || null)
  ...
  const handleDeleteFile = async () => {
    if (!user?.id || !savedFilePath) return

    const confirmed = window.confirm('Are you sure you want to delete this transcript file?')
    if (!confirmed) return
    ...
    toast.success('Transcript file deleted successfully')
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let transcriptFilePath = savedFilePath
    ...
    setSavedFilePath(transcriptFilePath)
            ...
            <CardContent>
              {loading ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <div className="space-y-2 text-sm text-muted-foreground">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {savedFilePath && (
                            <Button variant="outline" size="sm" asChild>
                              <a
                                href={getTranscriptUrl(savedFilePath)}
                                target="_blank"
                                rel="noreferrer"
                              >
                        ...
                        {!savedFilePath && (
                          <p className="text-destructive">
                            File path is missing. Please upload the transcript again.
                          </p>
                        )}
                      </div>
                    )}
                  ...
                  <Button type="submit" className="w-full" disabled={saving || deletingFile}>
                    {saving ? (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    ) : (
